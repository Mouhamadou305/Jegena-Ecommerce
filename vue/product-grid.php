            ...
            <div class="container">
                <div class="row">
                    <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
                        <div class="container">
                            <ul class="pagination">
                                <li class="page-item"><a class="page-link" href="product-grid?<?=isset($categorie)?'categorie='.htmlspecialchars($categorie->getId()):'' ?>&amp;page=<?=htmlspecialchars($page)>1?htmlspecialchars($page)-1:'1' ?>" style="color:#c43b68">Previous</a></li>
                                <?for($numPage=1; $numPage<=$nbrPages; $numPage++){?>
                                    <li class="page-item"><a class="page-link" href="product-grid?<?=isset($categorie)?'categorie='.htmlspecialchars($categorie->getId()):'' ?>&amp;page=<?=$numPage ?>" style="<?=htmlspecialchars($page)!=$numPage?'color:#c43b68':'background-color:#c43b68'?>"><?=htmlspecialchars($numPage) ?></a></li>
                                <?}?>
                                <li class="page-item"><a class="page-link" href="product-grid?<?=isset($categorie)?'categorie='.htmlspecialchars($categorie->getId()):'' ?>&amp;page=<?=htmlspecialchars($page)<$nbrPages?htmlspecialchars($page)+1:htmlspecialchars($nbrPages) ?>" style="color:#c43b68">Next</a></li>
                            </ul>
                        </div>
                                                        ...
                                                        if($article->getQuantite()>0){?>
                                                            ...
                                                            <div class="ht__list__product">
                                                                <div class="ht__list__thumb">
                                                                    <a href="product-details?article=<?=htmlspecialchars($article->getId())?>"><img src=<?=$manager->trouverImage($article)!==null && $manager->trouverImage($article)->getEmplacement()!==null?htmlspecialchars($manager->trouverImage($article)->getEmplacement()):"vue/images/product-2/pro-1/1.jpg"?> alt="product images"></a>
                                                                </div>
                                                                <div class="htc__list__details">
                                                                    <h2><a href="product-details?article=<?=htmlspecialchars($article->getId())?>"><?=htmlspecialchars($article->getNom()) ?> </a></h2>
                                                                    <ul  class="pro__prize">
                                                                        <li class="old__prize"><?=htmlspecialchars($article->getPrix())?> FCFA</li>
                                                                        <li><?=$article->isSolde()? htmlspecialchars($article->getPrix() - $article->getPrix()*($article->getPourcentageSolde()/100)):htmlspecialchars($article->getPrix())?> FCFA</li>
                                                                    </ul>                                   
                                                                    <p class="trunc"><?=htmlspecialchars($article->getDescription())?></p>
                                                                    <div class="fr__list__btn">
                                                                        <a class="fr__btn" href="<?= isset($_SESSION['compte'])?'cart?action=add&amp;article='.htmlspecialchars($article->getId()):'login?resultat=Connectez vous pour accéder au panier.' ?>">Add To Cart</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <!-- End List Product -->
                        ...
                        <div class="container">
                            <ul class="pagination">
                                <li class="page-item"><a class="page-link" href="product-grid?<?=isset($categorie)?'categorie='.htmlspecialchars($categorie->getId()):'' ?>&amp;page=<?=htmlspecialchars($page)>1?htmlspecialchars($page)-1:'1' ?>" style="color:#c43b68">Previous</a></li>
                                <?for($numPage=1; $numPage<=$nbrPages; $numPage++){?>
                                    <li class="page-item"><a class="page-link" href="product-grid?<?=isset($categorie)?'categorie='.htmlspecialchars($categorie->getId()):'' ?>&amp;page=<?=$numPage ?>" style="<?=htmlspecialchars($page)!=$numPage?'color:#c43b68':'background-color:#c43b68'?>"><?=$numPage ?></a></li>
                                <?}?>
                                <li class="page-item"><a class="page-link" href="product-grid?<?=isset($categorie)?'categorie='.htmlspecialchars($categorie->getId()):'' ?>&amp;page=<?=htmlspecialchars($page)<$nbrPages?htmlspecialchars($page)+1:htmlspecialchars($nbrPages) ?>" style="color:#c43b68">Next</a></li>
                            </ul>
                        </div>
